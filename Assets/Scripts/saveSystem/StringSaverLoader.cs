...
public class StringSaverLoader : MonoBehaviour
{
	public static StringSaverLoader Instance { get; private set; }
	...
	/// <param name="key"> ключ </param>
	/// <param name="username"> имя пользователя </param>
	public void Save(string value, string key, string username)
	{
		var bufKey = key + ":" + username;
		Save(value, bufKey);
	}
	...
	/// <param name="username"> имя пользователя </param>
	/// <returns> значение </returns>
	public string Load(string key, string username)
	{
		var bufKey = key + ":" + username;
		return Load(bufKey);
	}
	...
	public bool TryLoad(string key, out string value, out string error)
	{
		error = null;
	...
	public bool TryLoad(string key, string username, out string value, out string error)
	{
		var bufKey = key + ":" + username;
